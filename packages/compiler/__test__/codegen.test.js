...
describe('generateCode', () => {
  ...
  it('duplicate steps', () => {
    expect(() => generateCode(getDefinition('duplicate-ids'))).toThrowError(
      /duplicate step identifier/
    );
  });

  it('unknown type', () => {
    expect(() => generateCode(getDefinition('unknown-type'))).toThrowError(
      /unknown step type/
    );
  });
  ...
  it('should throw error when connection item is no object', () => {
    expect(() => generateCode(getDefinition('bad-connection'))).toThrowError(
      /connection expected as object/
    );
  });

  it('should throw error when connection reference unknown step id', () => {
    expect(() => generateCode(getDefinition('dst-id-no'))).toThrowError(
      /step not found with id/
    );
  });

  it('no exists start id for composite', () => {
    expect(() =>
      generateCode(getDefinition('composite-start-no'))
    ).toThrowError(/start id doesn't exists for composite/);
  });

  it('no exists end id for composite', () => {
    expect(() => generateCode(getDefinition('composite-end-no'))).toThrowError(
      /end id doesn't exists for composite/
    );
  });
