...
function generateAst(input, options = {}) {
  ...
  if (options.runtimeModule && scope.importClasses.size > 0) {
    if (options.runtimeModule) {
      ...
      statements.unshift(
        importFn(Array.from(scope.importClasses), options.runtimeModule)
      );
    }
  }
...
function generateNewInstance(varName, className, args = []) {
  return t.variableDeclaration('const', [
    t.variableDeclarator(
      t.identifier(varName),
      t.newExpression(t.identifier(className), args)
    )
  ]);
}
...
function generateImport(varName, moduleName) {
  return t.importDeclaration(
    Array.isArray(varName)
      ? varName.map(n => t.importSpecifier(t.identifier(n), t.identifier(n)))
  ...
  switch (kindOf(value)) {
    ...
    case 'array':
      return t.arrayExpression(
        value.map(val => generateValue(val, isSubstitutions, scope))
      );
    default:
  ...
  if (value.indexOf(REQUIRE_SUBSTITUTION) === 0) {
    ...
    if (scope.esModule) {
      ...
      return t.callExpression(t.identifier('require'), [
        t.stringLiteral(getModulePath(moduleName, scope))
      ]);
    }
  } else {
    return t.stringLiteral(value);
...
function generateObjProperty(
  propName,
  propValue,
  isSubstitutions = false,
  scope = null
) {
  return t.objectProperty(
    t.identifier(propName),
    generateValue(propValue, isSubstitutions, scope)
  );
}
...
function generateStepHandlerFromString(step, scope) {
  ...
  return t.objectProperty(
    ...
    t.callExpression(t.identifier(REQUIRE_FROM_STRING_VAR), [
  ...
  if (kindOf(props) === 'object') {
    return t.objectExpression(
      Object.keys(props).map(key =>
        generateObjProperty(key, props[key], true, scope)
      )
    );
  } else {
...
function generateStepConnection(stepId, connection, scope) {
  ...
  return t.expressionStatement(
    t.callExpression(
      ...
      [t.identifier(scope.stepVars[connection.stepId]), ...portArgs]
    )
  );
}
...
function generateAddStepsToPipeline(steps, scope) {
  return t.expressionStatement(
    t.callExpression(
      t.memberExpression(t.identifier(PIPELINE_VAR), t.identifier('add')),
      steps
...
function generateCompositeSubSteps(compositeStep, substeps, scope) {
  ...
  statements.push.apply(
    statements,
    generateNewInstancesOfSteps(substeps, compositeScope)
  );

  if (statements.length > 0) {
    statements.push.apply(
      statements,
      generateStepConnections(substeps, compositeScope)
    );
    statements.push.apply(
      statements,
      generateSetCompositeParameters(compositeStep, compositeScope)
    );
    statements.push(
      generateAddStepsToComposite(compositeStep, substeps, compositeScope)
    );
    statements.push.apply(
      statements,
      generateCompositeSteps(substeps, compositeScope)
    );
  }
...
function generateSetCompositeParameters(compositeStep, scope) {
  ...
  if (!scope.stepVars[compositeStep.startId]) {
    throw new Error(
      `start id doesn't exists for composite: ${compositeStep.startId}`
    );
  }
  if (!scope.stepVars[compositeStep.endId]) {
    throw new Error(
      `end id doesn't exists for composite: ${compositeStep.endId}`
    );
  }
