...
describe("ruleengine()", () => {
  describe("initialize", () => {
    it("bad args", async () => {
      expect(() => ruleengine(1)).toThrowError(/Rules argument must be an object or function/);
      expect(() => ruleengine(null)).toThrowError(/Rules argument must be an object or function/);
      expect(() => ruleengine(undefined)).toThrowError(/Rules argument must be an object or function/);
      expect(() => ruleengine(false)).toThrowError(/Rules argument must be an object or function/);
      expect(() => ruleengine("str")).toThrowError(/Rules argument must be an object or function/);
    });
  ...
  describe("debug mode", () => {
    it("enable debug", async () => {
      ...
      // eslint-disable-next-line no-unused-vars
      const server = createServer(rules, { debug: true, logger }, (req, res) => {
        req.params = { id: path.basename(req.url) }
      })
