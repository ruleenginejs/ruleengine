...
describe("base step", () => {
  ...
  it("pass bad id to options", () => {
    expect(() => { new Step({ id: false }) }).toThrow(TypeError);
    expect(() => { new Step({ id: [] }) }).toThrow(TypeError);
  });
  ...
  it("pass bad name to options", () => {
    expect(() => { new Step({ name: false }) }).toThrow(TypeError);
    expect(() => { new Step({ name: [] }) }).toThrow(TypeError);
    expect(() => { new Step({ name: 1 }) }).toThrow(TypeError);
  });
  ...
  it("pass bad handler to options", () => {
    expect(() => { new Step({ handler: false }) }).toThrow(TypeError);
    expect(() => { new Step({ handler: [] }) }).toThrow(TypeError);
    expect(() => { new Step({ handler: 1 }) }).toThrow(TypeError);
    expect(() => { new Step({ handler: "h" }) }).toThrow(TypeError);
  });
  ...
  it("disable not exists port", () => {
    ...
    expect(() => { step.enableInPort("port1", true) }).toThrowError(/port name/);
    expect(() => { step.enableOutPort("port2", false) }).toThrowError(/port name/);
    expect(() => { step.inPortEnabled("port1") }).toThrowError(/port name/);
    expect(() => { step.outPortEnabled("port2") }).toThrowError(/port name/);
  });
  ...
  it("get null connection", () => {
    const step1 = new Step({ ports: { in: ["port1"], out: ["port2", "port3"] } });
  ...
  it("get connection with bad src port", () => {
    ...
    expect(() => { step1.getConnection("port1") }).toThrowError(/doesn't exists/);
  });
  ...
  it("step connection to step with bad src port", () => {
    ...
    expect(() => { step1.connectTo(step2, "port1"); }).toThrowError(/doesn't exists/);
  });
  ...
  it("step connection to step with bad dst port", () => {
    ...
    expect(() => { step1.connectTo(step2, null, "port2"); }).toThrowError(/doesn't exists/);
  });
  ...
  it("unchecked dts port if connect with id", () => {
    ...
    expect(() => { step1.connectTo(step2.id, null, "port2"); }).not.toThrowError(/doesn't exists/);
  });
});
